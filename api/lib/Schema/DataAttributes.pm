package Schema::DataAttributes; #A table/row class
use 5.010001;
use strict;
use warnings;
use utf8;
use parent qw(Schema);

sub is_base_class{return 0}
my $TABLE_NAME = 'data_attributes';

sub TABLE {return $TABLE_NAME}
sub PRIMARY_KEY{return 'id'}
my $COLUMNS = [
             'id',
             'attribute_name',
             'attribute_title',
             'attribute_group',
             'data_type_id'
           ];

sub COLUMNS {return $COLUMNS}
my $ALIASES = {};

sub ALIASES {return $ALIASES}
my $CHECKS = {
            'id' => {
                      'defined' => 1,
                      'allow' => qr/^-?\d{1,20}$/x,
                      'required' => 1
                    },
            'attribute_group' => {
                                   'required' => 1,
                                   'allow' => sub { "DUMMY" },
                                   'defined' => 1
                                 },
            'data_type_id' => {
                                'defined' => 1,
                                'allow' => qr/^-?\d{1,20}$/x,
                                'required' => 1
                              },
            'attribute_name' => {
                                  'allow' => sub { "DUMMY" },
                                  'required' => 1,
                                  'defined' => 1
                                },
            'attribute_title' => {
                                   'allow' => sub { "DUMMY" }
                                 }
          };

sub CHECKS {return $CHECKS}

__PACKAGE__->QUOTE_IDENTIFIERS(0);
#__PACKAGE__->BUILD;#build accessors during load

1;

=pod

=encoding utf8

=head1 NAME

A class for TABLE data_attributes in schema db_Intranet2017

=head1 SYNOPSIS

=head1 DESCRIPTION

=head1 COLUMNS

Each column from table C<data_attributes> has an accessor method in this class.

=head2 id

=head2 attribute_name

=head2 attribute_title

=head2 attribute_group

=head2 data_type_id

=head1 ALIASES

=head1 GENERATOR

L<DBIx::Simple::Class::Schema>

=head1 SEE ALSO
L<Schema>, L<DBIx::Simple::Class>, L<DBIx::Simple::Class::Schema>

=head1 AUTHOR

david

=cut
