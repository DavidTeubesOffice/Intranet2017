package Schema::AuthenticationSessions; #A table/row class
use 5.010001;
use strict;
use warnings;
use utf8;
use parent qw(Schema);

sub is_base_class{return 0}
my $TABLE_NAME = 'authentication_sessions';

sub TABLE {return $TABLE_NAME}
sub PRIMARY_KEY{return 'id'}
my $COLUMNS = [
             'id',
             'session_token',
             'dbdate_created',
             'dbdate_modified',
             'authentication_entity_id'
           ];

sub COLUMNS {return $COLUMNS}
my $ALIASES = {};

sub ALIASES {return $ALIASES}
my $CHECKS = {
            'dbdate_created' => {
                                  'default' => 'CURRENT_TIMESTAMP',
                                  'defined' => 1,
                                  'required' => 1
                                },
            'session_token' => {
                                 'required' => 1,
                                 'allow' => sub { "DUMMY" },
                                 'defined' => 1
                               },
            'id' => {
                      'allow' => qr/^-?\d{1,20}$/x,
                      'required' => 1,
                      'defined' => 1
                    },
            'authentication_entity_id' => {
                                            'allow' => qr/^-?\d{1,20}$/x,
                                            'required' => 1,
                                            'defined' => 1
                                          },
            'dbdate_modified' => {
                                   'required' => 1,
                                   'defined' => 1
                                 }
          };

sub CHECKS {return $CHECKS}

__PACKAGE__->QUOTE_IDENTIFIERS(0);
#__PACKAGE__->BUILD;#build accessors during load

1;

=pod

=encoding utf8

=head1 NAME

A class for TABLE authentication_sessions in schema db_Intranet2017

=head1 SYNOPSIS

=head1 DESCRIPTION

=head1 COLUMNS

Each column from table C<authentication_sessions> has an accessor method in this class.

=head2 id

=head2 session_token

=head2 dbdate_created

=head2 dbdate_modified

=head2 authentication_entity_id

=head1 ALIASES

=head1 GENERATOR

L<DBIx::Simple::Class::Schema>

=head1 SEE ALSO
L<Schema>, L<DBIx::Simple::Class>, L<DBIx::Simple::Class::Schema>

=head1 AUTHOR

david

=cut
